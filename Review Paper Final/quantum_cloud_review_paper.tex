...
\begin{abstract}
This paper discusses a new Cross- Platform Function as a Service -XFaaS- that is all about allowing quantum workloads to execute well on AWS Lambda, Azure Functions and Google Cloud Functions. XFaaS eliminates the pain of relying on a single vendor by providing you with additional fault tolerance, the opportunity to compare the performance, and a certain savings through the intelligent coordination of a group of providers. In fact, we are able to execute quantum algorithms on various cloud services, with AWS Braket, Qiskit simulators, and Cirq. Such an arrangement ensures that results are always aggregated and analyzed. Testing indicates that XFaaS is solid rock, and its uptime is 99.7 percent as compared to 99.2 percent of single-provider setups. It has cross platform run times of 2.1 to 3.4 seconds and maintains results with more than 95% correlation. In case a provider hiccups, quantum computations are maintained, and typically, switching over takes an average of only 2.3 seconds. Overall, XFaaS is a sound enterprise quantum computing strategy that provides us with actual information about the reliability and consistency of multi-cloud quantum processing.
\end{abstract}
